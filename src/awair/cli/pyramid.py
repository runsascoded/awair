...
@pyramid.command('stats-backfill')
@option('-n', '--pyramid-name-prefix', 'pyramid_name_prefix', type=str, default='awair', help='Prefix for per-device pyramid names (rows use `{prefix}-{device_id}`)')
@option('-o', '--out', 'out_path', type=str, default='-', help="Output SQL file ('-' for stdout, default)")
@option('-b', '--bucket', type=str, default='awair', help='R2 bucket to enumerate (default: awair)')
@option('-p', '--prefix', type=str, default='pyramid/', help="Key prefix to walk (default: 'pyramid/')")
def stats_backfill(
    ...
    """Walk R2 pyramid shards, read each parquet's metadata, and emit
    UPDATE SQL to fill `size_bytes / n_rows / n_rgs / rg_row_counts /
    footer_bytes` columns on `pyramid_shards`.

    `footer_bytes` holds the last 64 KiB of each shard file — pyrmts'
    `DEFAULT_INITIAL_FETCH_SIZE`. `cfw/serve /q` serves it from D1 when
    the request's `end` matches D1's `size_bytes` (i.e. the file hasn't
    grown since cache was written), skipping the R2 metadata round-trip.

    Cascade stamps these at write time (see `cascade.ts::recordShard`
    follow-up UPDATE), so this one-shot backfill only needs to run for
    shards that predate the stats migration (`0003_shard_stats.sql` +
    `0004_footer_cache.sql`) or that are written by Lambda (the raw tier
    — cascade doesn't own raw writes; raw cache entries will invalidate
    every minute as Lambda extends the file, which is fine).

    Apply:

        awair pyramid stats-backfill -o /tmp/stats.sql
        cd cfw/cascade && pnpm wrangler d1 execute awair-cascade --remote --file /tmp/stats.sql
    """
    ...
    r2 = _r2_client()
    paginator = r2.get_paginator('list_objects_v2')
    updates: list[tuple[str, str, str, int, int, int, int, list[int], bytes]] = []
    #                  ^pyramid ^tier ^shard   ^p_start ^size ^rows ^rgs ^rg_rows ^footer
    scanned = 0
    ...
    for page in paginator.paginate(Bucket=bucket, Prefix=prefix):
        for obj in page.get('Contents', []):
            ...
            f = pq.ParquetFile(BytesIO(body))
            n_rgs = f.metadata.num_row_groups
            n_rows = f.metadata.num_rows
            rg_rows = [f.metadata.row_group(i).num_rows for i in range(n_rgs)]
            pyramid_name = f'{pyramid_name_prefix}-{device_id}'
            # Cache the *actual* last 64 KiB (or the whole file if smaller);
            ...
            if scanned % 20 == 0:
                err(f'  scanned {scanned} shards…')

    err(f'\nBuilt updates for {len(updates)} shard(s) (scanned {scanned}) → {out_path}')
    stream = sys.stdout if out_path == '-' else open(out_path, 'w')
    try:
        stream.write(
            f'-- Backfill per-shard stats + footer cache on `pyramid_shards`.\n'
            f'-- Generated by `awair pyramid stats-backfill`. Statements are idempotent.\n\n'
        )
        for pyr, tier_name, shard_dur, p_start_ms, size, rows, rgs, rg_rows, footer in updates:
            import json
            stream.write(
                'UPDATE "pyramid_shards" SET '
                f'size_bytes = {size}, n_rows = {rows}, n_rgs = {rgs}, '
                f"rg_row_counts = '{_sql_str(json.dumps(rg_rows))}', "
                f"footer_bytes = x'{footer.hex()}' "
                f"WHERE pyramid = '{_sql_str(pyr)}' AND tier = '{_sql_str(tier_name)}' "
                f"AND shard_dur = '{_sql_str(shard_dur)}' AND period_start = {p_start_ms};\n"
            )
    finally:
        if stream is not sys.stdout:
            stream.close()
