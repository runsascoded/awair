...
def stats_backfill(
    ...
    or that are written by Lambda (the raw tier — cascade doesn't own
    raw writes).
    ...
    r2 = _r2_client()
    paginator = r2.get_paginator('list_objects_v2')
    updates: list[tuple[str, str, str, int, int, int, int, list[int]]] = []
    #                  ^pyramid ^tier ^shard   ^p_start ^size ^rows ^rgs ^rg_rows
    scanned = 0
    ...
    try:
        stream.write(
            f'-- Backfill per-shard stats on `pyramid_shards`.\n'
            f'-- Generated by `awair pyramid stats-backfill`. Statements are idempotent.\n\n'
        )
        for pyr, tier_name, shard_dur, p_start_ms, size, rows, rgs, rg_rows in updates:
            import json
            stream.write(
